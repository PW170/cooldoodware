...
# We just inject before GradientUtil but declare extraText earlier
old_watermark = r'GradientUtil\.applyGradientHorizontal\(0, 0, width, height, 1, color1, color2, \(\) -> \{\s*FontUtil\.drawString\(CLIENT_NAME, 0, 0, size, Color\.WHITE, true\);\s*\}\);\s*int extraSize = \(int\) \(size / 2\.0\);\s*String extraText = " \| " \+ C\.mc\.getDebugFPS\(\) \+ "fps \| " \+ getPing\(\) \+ "ms";'

new_watermark = '''int extraSize = (int) (size / 2.0);
