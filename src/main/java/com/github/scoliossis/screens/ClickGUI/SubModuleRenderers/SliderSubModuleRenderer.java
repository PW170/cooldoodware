...
public class SliderSubModuleRenderer extends SubModuleRenderer {
    ...
    public void render(int mouseX, int mouseY, SubModule subModule) {
        ...
        RenderUtil.drawRoundedRect(cardX + 2, trackY, cardW - 4, SLIDER_H, SLIDER_H / 2f,
        ...
        if (fillW > 0) {
            RenderUtil.drawRoundedRect(cardX + 2, trackY, fillW, SLIDER_H, SLIDER_H / 2f,
                    new Color(catColor.getRed(), catColor.getGreen(), catColor.getBlue(), 220));
        }

        // Thumb dot
        float thumbX = cardX + 2 + fillW - (SLIDER_H + 2) / 2f;
        thumbX = MathHelper.clamp_float(thumbX, cardX + 2, cardX + cardW - 4 - SLIDER_H - 2);
        RenderUtil.drawRoundedRect(thumbX, trackY - 1, SLIDER_H + 2, SLIDER_H + 2,
                (SLIDER_H + 2) / 2f, Color.WHITE);
